...
import { useLeaves } from "@/app/features/leaves/hooks/useLeaves";
...
export default function AdminLeavesPage() {
  ...
  const action = async (id: string, act: "approve" | "reject") => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API}/leaves/${id}/${act}`, {
        method: "PUT",
        headers,
      });
      const json = await res.json();
      ...
      <main className="ml-4 w-full p-4 sm:p-6 lg:p-8">
        ...
        <div className="mb-4 flex flex-wrap gap-3">
          ...
          >
            ...
            {["sick", "casual", "earned", "other"].map((t) => (
                ...
                <tbody className="divide-y divide-slate-50 bg-white">
                  {filtered.map((l) => (
                    <tr key={l._id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-2 text-center font-medium text-slate-800">
                        {l.numberOfDays}
                      </td>
                      <td className="max-w-xs px-4 py-2 truncate text-slate-500">
                        {l.reason}
                      </td>
                      <td className="px-4 py-2">
                        <span
                          className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${STATUS_COLORS[l.status]}`}
                        >
                          {l.status}
                        </span>
                      </td>
